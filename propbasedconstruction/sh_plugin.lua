...
if (CLIENT) then
	...
	function PLUGIN:HUDPaint()
		...
		if lclient:GetNWBool("ConstructablePropPlacing", false) then
			...
			draw.SimpleText("MODE DE PLACEMENT ACTIVÉ", "ConstructionPropFont", w/2, h/7, Color(255, 0, 0, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
			
			if lclient:GetActiveWeapon():GetClass() != "ix_hands" then
				draw.SimpleText("Les mains doivent être équipées pour commencer le placement", "ConstructionPropFont", w/2, h/1.155, Color(255, 150, 150, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
			end
		
			if lclient:GetActiveWeapon():GetClass() == "ix_hands" then
				draw.SimpleText("LMB: Inclinaison vers l'avant | RMB: Incliner à droite | " .. string.upper(useBind) .. ": Placer | " .. string.upper(reloadBind) .. ": Quitter le placement", "ConstructionPropFont", w/2, h/1.155, Color(255, 255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
			end
		end
	end
end
