...
function PANEL:Init()
	...
	self.icon:Dock(LEFT)
	self.icon:SetWide(Schema:RX(25))

	self.text = ""
	self.tickSound = "weapons/ar2/ar2_empty.wav"
...
function PANEL:Think()
	...
	if (time >= self.nextThink) then
		if (self.character < self.text:utf8len()) then
			...
			self.label:SetText(" | "..string.utf8sub((self.text):utf8upper(), 1, self.character))
			self.label:SetFont("CHud2.5")

			LocalPlayer():EmitSound(self.tickSound, 70, math.random(self.tickSoundRange[1], self.tickSoundRange[2]))
		end
...
function PANEL:AddEntry(entry, color, icon)
