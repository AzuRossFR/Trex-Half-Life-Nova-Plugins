...
function PANEL:Init()
	...
	self:SetTitle(("Nouvelle Zone"):utf8upper())
	...
	listRow:SetLabelText(L("name"):utf8upper())
	listRow:SetLabelColor(color_buttonapo)
	...
	self.typeEntry:SetFont("CHud2.5")
	...
	listRow:SetList(self.list)
	listRow:SetLabelText(L("type"):utf8upper())
	listRow:SetLabelColor(color_buttonapo)
	...
	for k, v in pairs(ix.area.properties) do
		...
		if (IsValid(panel)) then
			...
			row:SetLabelText(L(k):utf8upper())
			row:SetLabelColor(color_buttonapo)
	...
	-- save button
	self.saveButton = self:Add("ixMenuSelectionButtonTrex")
	self.saveButton:SetText(L("save"):utf8upper())
	self.saveButton:SetFont("CHud2")
	self.saveButton:SetTextColor(color_buttonapo)
