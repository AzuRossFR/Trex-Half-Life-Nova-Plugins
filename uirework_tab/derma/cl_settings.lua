...
function PANEL:SetShowReset(value, name, default)
	...
	if (value and !IsValid(self.reset)) then
		...
		self.reset:SetFont("ixIconsMedium")
		...
		self.reset:SetHelixTooltip(function(tooltip)
			...
			description:SetText(L("resetDefaultDescription", tostring(L(name)), tostring(default)))
...
function PANEL:AddCategory(name)
	...
	if (!IsValid(panel)) then
		...
		panel.Paint = function(s,w,h)
			...
			draw.DrawText(string.utf8upper(name),"CHud2.5",0,2)
