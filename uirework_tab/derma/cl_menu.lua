...
function PANEL:Init()
	...
	self.buttons:SetSize(self:GetWide(), 61)
	self.buttons:SetPos(0,0)
	self.buttons:SetPaintedManually(true)
	self.buttons.Paint = function(s,w,h)
		surface.SetDrawColor(ColorAlpha(color_white,self.currentAlpha * 0.2))
		surface.DrawRect(0, 60, w,1)
	end


	local characters = self.buttons:Add("ixMenuSelectionButtonTrex")
	characters:SetText("/")
	...
	characters.DoClick = function()
		...
		LocalPlayer():ScreenFade(0,color_black,1,1)
	end
...
function PANEL:GetStandardSubpanelSize()
	return ScrW() * 0.95 , ScrH() - self:GetPadding() * 2
end
...
function PANEL:SetupTab(name, info, sectionParent)
	...
	if (sectionParent) then
		...
		button = self.tabs:Add("ixMenuSelectionButtonTrex")
		button:SetText(L(name))
		button:Dock(LEFT)
		button:SetFont("CHud2.5")
...
function PANEL:Remove()
	...
	if IsValid(ix.gui.limbpanel) then
		ix.gui.limbpanel:Close()
	end
	CloseDermaMenus()
