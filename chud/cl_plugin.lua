...
local function fCitizen( client )
  ...
  local status = !bol and "MONITEUR" or "RECHERCHÉ"
...
local function fMetropolice( client )
  ...
  local useTag = ix.config.Get( "Utiliser les titres d'appel", false )
  cam.PushModelMatrix( m )
    draw.SimpleText( "<:: Rang de l'unité: " .. rank .. " ::>", "CHudLabel", client.toScreenX, client.toScreenY + 30, col_white, TEXT_ALIGN_CENTER )
    if ( useTag ) then
      draw.SimpleText( "<:: ID de l'unité: " .. tag .. "-" .. id .. " ::>", "CHudLabel", client.toScreenX, client.toScreenY, col_white, TEXT_ALIGN_CENTER )
    else
      draw.SimpleText( "<:: ID de l'unité: " .. id .. " ::>", "CHudLabel", client.toScreenX, client.toScreenY, col_white, TEXT_ALIGN_CENTER )
    end
  cam.PopModelMatrix()
  return inRange, mult
end
