...
function PANEL:Init()
  ...
  self.list:AddColumn( "Identification de l'unité" )
  self.list:AddColumn( "Rang" ):SetFixedWidth( 150 )
  ...
  dock.socio:SetText( "Définir le statut social" )
  ...
  local status = {
    "STABLE", "MARGINALE", "INFECTION", "FRACTURE", "MINUIT"
  }
  ...
  dock.cid:SetText( "Gérer les CID" )
  ...
  function dock.cid:DoClick()
    ...
    frame:Center()
    frame:MakePopup()
    frame:SetTitle( "Gestionnaire de CID" )
    ...
    for i, v in ipairs( player.GetAll() ) do
      ...
      button:Dock( TOP )
      button:SetSize( 0, 90 )
      button:SetText( "Nom du CID: " .. v:GetName() .. "\nCID: " .. v:GetNetVar( "cid", "NULL" ) )
      ...
      function button:DoClick()
        Derma_StringRequest( "Nouveau CID", "Définir le nouveau CID de ce citoyen", v:GetNetVar( "cid", "NULL" ), function( text )
          if ( !string.match( text, "%d%d%d%d%d" ) ) then
            LocalPlayer():Notify( "Vous devez spécifier un numéro de CID valide !" )
            return
          end
  ...
  dock.rank:SetText( "Définir le rang" )
