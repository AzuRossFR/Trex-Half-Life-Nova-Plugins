...
function ENT:Use(act, cal)
	...
	local char = cal:GetCharacter()
	local nb = "Vous avez reçu <money> pour avoir lavé <clothes> vêtements"
	char:AddLevelXP(#self.ClothTable * 100)

	local phrase = string.gsub(nb, "<clothes>", tostring(#self.ClothTable))
	phrase = string.gsub(phrase, "<money>", tostring(#self.ClothTable * 100) .. "XP")

	ix.util.Notify(phrase, cal)
