...
function ENT:Initialize()
  self:SetModel("models/trexhln/combine_fabricator.mdl")
...
function ENT:OnWash(ent, name, ov, nv)
  ...
  timer.Create("WashTimer" .. self:EntIndex(), 1, 15, function()
    self:SetWashState(self:GetWashState() - 1)

    timer.Simple(1, function()
      if self:IsValid() then
        self:EmitSound("plats/elevator_stop1.wav", 20)
      end
    end)
    ...
    if self:GetWashState() == 0 then
      ...
      cloth:SetPos(pos + (ang:Right() * -40))
      cloth:SetAngles(self:GetAngles() + Angle(50,0,0))
