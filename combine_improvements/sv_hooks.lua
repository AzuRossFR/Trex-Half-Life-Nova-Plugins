...
function PLUGIN:PlayerLoadedCharacter(client, character, currentChar)
  if character:IsCombine() then
    ...
    if character:GetFaction() == FACTION_MPF then
      Schema:AddCombineDisplayMessage("Toutes les unités doivent être conscientes du biosignal cohésif émis par la " .. character:GetName() .. " a été reçue...", Color(220, 0, 255))
    elseif character:GetFaction() == FACTION_OTA then
      Schema:AddCombineDisplayMessage("Toutes les unités doivent être conscientes qu'Overwatch s'est déployé à partir de la stase...", Color(220, 0, 255))
    end
...
net.Receive("ChangedStatus",function(_, send)
  if send:IsCombine() then
    ...
    Schema:AddCombineDisplayMessage(ply:GetName() .. " #" .. ply:GetNWString("cid", "NO CID") .. "L'identifiant du citoyen a été mis à jour pour devenir " .. status .. "...", Colours[status])
...
net.Receive("ChangedCID",function(_, send)
  if send:IsCombine() then
    ...
    Schema:AddCombineDisplayMessage(ply:GetName() .. "L'identifiant du citoyen a été mis à jour pour devenir " .. cid .. "...")
    CombSound()
  end
end)
...
net.Receive("SocioStatus",function(_, ply)
  if ply:IsCombine() then
    ...
    Schema:AddCombineDisplayMessage("Le statut social a été mis à jour pour : " .. NewStatus .. "...!", PLUGIN.SocioStatusCol[NewStatus])
...
net.Receive("BOL",function(_, send)
  if send:IsCombine() then
    ...
    if bool == true then
      ...
      Schema:AddCombineDisplayMessage("Citizen " .. ply:GetName() .. " #" .. ply:GetCharacter():GetData("cid", "ERR NO CID") .. " a été ajouté à la liste BOL...!" , Color(255,0,0))
    else
      table.RemoveByValue(PLUGIN.BOL, ply:GetName())
      Schema:AddCombineDisplayMessage("Citizen " .. ply:GetName() .. " #" .. ply:GetCharacter():GetData("cid", "ERR NO CID") .. " a été retirer de la liste BOL...!")
    end
...
net.Receive("NewNotes", function(_, ply)
  if ply:IsCombine() then
    ...
    Schema:AddCombineDisplayMessage("Citizen " .. Citizen:GetName() .. " #" .. Citizen:GetCharacter():GetData("cid", "ERR NO CID") .. " a fait l'objet d'une mise à jour de ses notes...!")
    CombSound()
  end
end)
...
net.Receive("Promotion", function(_, send)
  if send:IsCombine() then
    ...
    Schema:AddCombineDisplayMessage(name .. "'à était promu au rang de " .. rankTable[1] .. "...")
    CombSound()
...
net.Receive("Citizenship", function(_, ply)
  ...
  if ply:IsCombine() then
    ...
    if type == true then
      ...
      Schema:AddCombineDisplayMessage(ent:GetName() .. " a été déchu de sa citoyenneté et est désormais considéré comme un anti-citoyen, évaluation : Effacer...", Color(255,0,0))
      CombSound()
    else
      ...
      Schema:AddCombineDisplayMessage(ent:GetName() .. " a vu sa citoyenneté rétablie...")
      CombSound()
    end
